 ...
 *
 * Philosophy: don't predict task difficulty upfront. Start at the user's
 * (or config's) baseline effort, and let the model escalate only when the
 * work proves harder than expected.
 *
 * Mechanics (all public plugin API, no core changes):
 * - `chat.message` resets effort to baseline on every new user message (task boundary).
 * - `Hooks.tool` registers a `request_effort` tool the model calls to signal
 *   that it needs more reasoning. Escalation is monotonic (never decreases),
 *   capped a couple of rungs above baseline, and rate-limited per task.
 ...
 *   model knows the tool exists and when to use it.
 *
...
type State = {
  ...
  /** Escalations granted for the current task. */
  escalations: number
}

const MAX_ESCALATIONS_PER_TASK = 2

/** Session state. Effort moves upward only; a new user message resets it. */
...
function escalate(sessionID: string, requested: Effort): { granted: Effort; changed: boolean; note: string } {
  const entry = state.get(sessionID) ?? { escalated: undefined, escalations: 0 }
  const current = entry.escalated ?? "low"
...
export const TaskEffortRouterPlugin = (async () => {
  return {
    "chat.message": async (input) => {
      // New user message = new task boundary: effort returns to baseline.
    ...
    "chat.params": async (input, output) => {
      ...
      const tier = entry.escalated

      // Only raise: if the effective options already pin an effort at or above
      // the requested tier (user selection / agent config), leave them alone.
