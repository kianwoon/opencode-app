...
You can configure the brain orchestration agents through the `brain` option.
...
- `model` - Model for the primary `brain` agent, in `provider/model` format.
- `hands_model` - Model for the generated `explorer` and `implementer` subagents.
- `reviewer_model` - Model for the generated `reviewer` subagent.
- `enforcement` - `strict` gives the generated agents restrictive permissions that take precedence over the global `permission` block; `advisory` (default) only warns without blocking.

Generation is fill-absent-only: any agent you define yourself in the [`agent`](#agents) block takes precedence over a generated one.
