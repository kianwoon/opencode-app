...
it.instance("auto title keeps the title when follow-ups stay on the same topic", () =>
  Effect.gen(function* () {
    const { llm } = yield* useServerConfig(providerCfg)
    const prompt = yield* SessionPrompt.Service
    const sessions = yield* Session.Service
    ...
    // A substantive follow-up that stays on the same topic: the title request
    // runs again but must return the current title unchanged.
    yield* llm.pushMatch(titleMatch, reply().text("First Topic").stop())
    yield* llm.push(reply().text("main two").stop())
    yield* prompt.prompt({
      sessionID: chat.id,
      agent: "build",
      parts: [{ type: "text", text: "and one more question about the same first topic" }],
    })
    yield* llm.wait(4)
    expect((yield* sessions.get(chat.id)).title).toBe("First Topic")
  }),
