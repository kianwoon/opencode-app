...
describe("ProviderTransform.providerOptions - openrouter per-model routing", () => {
  const sessionID = "test-session-123"
  ...
  test("translates model-level routing.only to provider.only", () => {
    const result = ProviderTransform.providerOptions(model, { routing: { only: ["p1"] } })
    expect(result).toEqual({ openrouter: { provider: { only: ["p1"] } } })
  })

  test("routing overlays provider at model level", () => {
    const result = ProviderTransform.providerOptions(model, {
      provider: { only: ["p1"], zdr: true },
      routing: { only: ["p2"] },
    })
    expect(result).toEqual({ openrouter: { provider: { only: ["p2"], zdr: true } } })
  })

  test("drops invalid sort and removes the routing key from output", () => {
    const result = ProviderTransform.providerOptions(model, {
      routing: { sort: "bogus", only: ["p1"], rogue: "x" },
    })
    expect(result).toEqual({ openrouter: { provider: { only: ["p1"] } } })
  })

  test("preserves existing provider subkeys and drops routing when no whitelist keys match", () => {
    const result = ProviderTransform.providerOptions(model, {
      provider: { custom_thing: 1 },
      routing: { rogue: "x" },
    })
    expect(result).toEqual({ openrouter: { provider: { custom_thing: 1 } } })
  })

  test("provider-level base options still work when no model routing override", () => {
    const base = ProviderTransform.options({
      model,
      sessionID,
      providerOptions: { routing: { only: ["base-provider"] } },
    })
    // request.ts merge order: base (provider.options) ← model.options ← agent.options ← variant,
    // so model-level routing wins over the provider-level pin.
    const merged = mergeDeep(base, { routing: { only: ["p1"] } }) as Record<string, any>
    const result = ProviderTransform.providerOptions(model, merged)
    expect(result.openrouter).toMatchObject({ provider: { only: ["p1"] } })
    expect((result.openrouter as any).routing).toBeUndefined()
  })

  test("empty routing.only clears an existing pin instead of preserving it", () => {
    const result = ProviderTransform.providerOptions(model, {
      provider: { only: ["streamlake"], zdr: true },
      routing: { only: [] },
    })
    expect(result).toEqual({ openrouter: { provider: { zdr: true } } })
  })

  test("empty routing.order clears an existing order instead of preserving it", () => {
    const result = ProviderTransform.providerOptions(model, {
      provider: { order: ["p1"], only: ["p1"] },
      routing: { order: [] },
    })
    expect(result).toEqual({ openrouter: { provider: { only: ["p1"] } } })
  })

  test("empty string data_collection/ignore clears instead of preserving", () => {
    const result = ProviderTransform.providerOptions(model, {
      provider: { data_collection: "deny", ignore: "p1" },
      routing: { data_collection: "", ignore: "" },
    })
    expect(result).toEqual({ openrouter: {} })
  })

  test("absent only/order keys still preserve existing provider subkeys", () => {
    const result = ProviderTransform.providerOptions(model, {
      provider: { only: ["p1"], order: ["p2"] },
      routing: { sort: "price" },
    })
    expect(result).toEqual({ openrouter: { provider: { only: ["p1"], order: ["p2"], sort: "price" } } })
  })

  test("override test: model routing.only replaces provider-level pin through options() merge", () => {
    const base = ProviderTransform.options({
      model,
      sessionID,
      providerOptions: { routing: { only: ["streamlake"] } },
    })
    const merged = mergeDeep(base, { routing: { only: ["deepinfra"] } }) as Record<string, any>
    const result = ProviderTransform.providerOptions(model, merged)
    expect((result.openrouter as any).provider.only).toEqual(["deepinfra"])
  })

  test("override test: empty model routing.only clears the provider-level pin entirely", () => {
    const base = ProviderTransform.options({
      model,
      sessionID,
      providerOptions: { routing: { only: ["streamlake"] } },
    })
    const merged = mergeDeep(base, { routing: { only: [] } }) as Record<string, any>
    const result = ProviderTransform.providerOptions(model, merged)
    expect((result.openrouter as any).provider?.only).toBeUndefined()
  })

  test("no routing or provider key passes options through untouched", () => {
    const result = ProviderTransform.providerOptions(model, { prompt_cache_key: "abc" })
    expect(result).toEqual({ openrouter: { prompt_cache_key: "abc" } })
  })
})
