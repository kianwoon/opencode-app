  ...
  compaction: Schema.optional(
    Schema.Struct({
      ...
      trigger_tokens: Schema.optional(NonNegativeInt).annotate({
        description:
          "Total-token threshold that triggers compaction, overriding the model-derived fire point (the model's usable window). Unset keeps the model-derived behaviour.",
      }),
  ...
  experimental: Schema.optional(
    Schema.Struct({
      ...
      tool_search_threshold: Schema.optional(Schema.Finite).annotate({
        description:
          "Fraction of the model context window (0-1) that MCP tool definitions may occupy before they defer behind the find_tools tool. 0 defers immediately, 1 never defers. Default 0.1.",
      }),
