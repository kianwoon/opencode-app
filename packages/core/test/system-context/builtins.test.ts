...
describe("SystemContextBuiltIns", () => {
  it.effect("loads a stable location-scoped environment baseline", () =>
    Effect.gen(function* () {
  ...
  it.effect("baseline excludes volatile content so the cache prefix stays stable", () =>
    Effect.gen(function* () {
      const context = yield* SystemContextRegistry.Service
      const initialized = yield* SystemContext.initialize(yield* context.load())

      expect(initialized.baseline).not.toContain("Today's date")
      expect(initialized.baseline).not.toContain(new Date().toDateString())
    }),
  )
